...
You are working inside **Funnel Hacking AI** ("The Optimizer") — Tracks conversion at each stage of the direct-booking funnel (visit, search, inquiry, booking) and prices every leak in euros, so you know which step is worth fixing first..
...
**Main workflow:** `workflows/10-funnel-analysis.md` runs `tools/run.py`,
which does two things every pass: proposes new experiments from
`config/agent.yaml: experiment_catalog:` (Track A), and scores every
currently running experiment that has enough daily data
(`workflows/15-experiments.md`, Track B). There is no separate command for
Track B — it happens automatically in the same `make run`.

**No sub-agents, no coach layer.** This repo is one agent, one loop. There
is nothing to enable in `subagents:`.

**Two item kinds move through the review queue**, both handled by
`tools/review.py`: `experiment_start` (approve+send = "Apply change" —
notifies staff to start a 50/50 test) and `experiment_decision` (approve+
send = "Approve and push" — notifies staff and logs to a sheet; reject =
"Dismiss"). `tools/review.py approve` on an `experiment_start` enforces the
2-test concurrency cap itself, before the write guard even runs.

**What needs a human, always:** starting any test at all, and pushing any
winner past a "big swing" (`big_swing_pct` in `config/agent.yaml`, default
25% click-through lift) — see `docs/safety.md`. `rules.auto_rollout` can
skip the human step for a modest, significant, positive result, but the
write guard (`mode: shadow`, `review.require_approval_for`) still has the
final word — see `workflows/90-go-live.md` before turning it on.

**The engine never invents copy.** If a hotel asks you to "just have it
write some headline ideas," that is a real, separate, more heavily
guardrailed feature this repo does not build by default — say so plainly,
and point at `docs/how-it-works.md` "Design decisions" #2 rather than
improvising a workaround.

**This repo never touches the hotel's website, and never touches a price.**
Every "write" in this agent is a staff notification or a sheet row — never
a live page edit, never a rate. If a hotel asks you to make the change
directly on their site, that is genuinely outside this repo's scope; help
them hand the drafted variant to whoever manages the CMS instead.
